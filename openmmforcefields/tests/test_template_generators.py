...
def smirnoff_small_molecule_forcefields():
    force_fields = list()
    for small_molecule_forcefield in SMIRNOFFTemplateGenerator.INSTALLED_FORCEFIELDS:
        ...
        if 'opc' in small_molecule_forcefield:
            continue
        if small_molecule_forcefield.startswith('ff14sb'):
            continue
        force_fields.append(small_molecule_forcefield)
...
class TestSMIRNOFFTemplateGenerator(TestGAFFTemplateGenerator):
    ...
    @pytest.mark.parametrize("small_molecule_forcefield", smirnoff_small_molecule_forcefields())
    def test_energies(self, small_molecule_forcefield):
    ...
    @pytest.mark.parametrize("small_molecule_forcefield", smirnoff_small_molecule_forcefields())
    def test_partial_charges_are_none(self, small_molecule_forcefield):
    ...
    @pytest.mark.skip
    def test_parameterize(self, small_molecule_forcefield):
        """Redundant inherited test."""
